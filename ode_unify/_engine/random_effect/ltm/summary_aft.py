"""Port of random effect/ltm/summary_aft.m.

Aggregates ``rep`` replications of the AFT-frailty LTM fit, falling back
from ``K4`` to ``K3`` when the K4 fit failed.  Filters out replications
whose resampling SE is unreasonably small/large (``se_all(:,1)`` outside
``(0, 0.15)``) — matches the MATLAB convention.
"""
...
def summary_aft(N=2000, rep=1000, data_setting=2, knots_setting='K4',
                root=None):
    if root is None:
        root = os.path.dirname(__file__)
    res_sub = 'aft_rd'; res_prefix = 'res_aft_N'
    res_dir = os.path.join(root, 'res', res_sub)

    est_params = np.zeros((rep, 20))
    est_se_all = np.zeros((rep, 20))
    ...
    for seed in range(1, rep + 1):
        ...
        n_store = min(20, est_r.size - 1)
        est_params[seed - 1, :n_store] = est_r[1:1 + n_store]
        est_se_all[seed - 1, :se_all.size] = se_all[:20]

    idx = (est_se_all[:, 0] > 0) & (est_se_all[:, 0] < 0.15)
    ...
    try:
        ...
        true_a_log = np.zeros_like(pxt)
        true_q_log = np.log(2.0 / (1.0 + pxq))
        ...
        axes[0].plot(pxt, all_al.mean(0), '-.',
                     color=(0.929, 0.694, 0.125), lw=2)
        axes[0].set_xlim(0, 2); axes[0].set_ylim(-4, 4)
        ...
        axes[1].plot(pxq, all_ql.mean(0), '-.',
                     color=(0.929, 0.694, 0.125), lw=2)
        axes[1].set_xlim(0, 10); axes[1].set_ylim(-4, 4)
        axes[1].set_xlabel('u'); axes[1].set_ylabel('q(u)')
        out_png = os.path.join(root, f'ltm_aft_rd_{N}_rep{rep}.png')
