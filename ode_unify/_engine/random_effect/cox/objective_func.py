"""Port of random effect/cox/objective_func.m.

Negative log partial likelihood under a Cox-type intensity.  Baseline
hazard ``lambda_0(t) = exp(B(t) @ theta)``.  Subject-level random effects
are integrated out implicitly by the recurrent-event likelihood; no
``xi`` appears here -- that only affects the data-generation process.
"""
...
def objective_func(r, x, time, delta, knots, k):
    ...
    grad_beta = -(delta @ x) + (cum_baseline_hazard * multi_coef) @ x_tau
    grad_theta_part1 = -(delta @ B)
    ...
    grad_theta = grad_theta_part1 + multi_coef @ dtheta2
