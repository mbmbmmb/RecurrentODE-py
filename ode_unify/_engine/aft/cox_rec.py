"""Port of aft/cox_rec.m.

Fits the Cox-type objective to get an initial beta and the per-subject
cumulative baseline hazard ``temp``, used to seed the AFT routine.
"""
...
def cox_rec(x, time, delta):
    ...
    r0 = np.zeros(p + q)
    result = minimize(fun, r0, jac=True, method='BFGS',
                      options={'maxiter': 500})
    est_r = result.x
