"""Driver for the uniform-censoring AFT recurrent-event simulation.

Identical fit/inference code as ``RecurrentODE_py.aft.main`` -- only the
data generator is swapped to ``inform_censor.aft_unif.generator_rec``,
which draws C|x ~ Uniform(a(x), b(x)).
"""
