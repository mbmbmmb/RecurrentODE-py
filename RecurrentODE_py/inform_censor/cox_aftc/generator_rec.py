...
* Censoring time C|x is drawn from an AFT-type survival model with the
  ...
  Conditionally (given x) C is independent of the recurrent event process,
  but marginally C and N(.) are correlated through x.
* Fixed upper bound TAU caps follow-up at min(C, TAU). Imperceptible
  ...
  below the numerical floor, so that the downstream solve_ivp sees a
  strictly-increasing t_eval. lambda_0_C tuned so that the median of C
  at x=0 is approx 2.33 (matches the Cox-survival variant).
"""
...
def generator_rec(N, seed, data_setting, data_dir=None,
    ...
    # AFT-type censoring: invert H_C = sqrt(4 a exp(x'g) t + 1) - 1.
