"""Driver for the informative-censoring AFT recurrent-event simulation.

Identical fit/inference code as ``RecurrentODE_py.aft.main`` -- we only
swap the data generator for ``inform_censor.aft.generator_rec`` so that
...
from ...aft.cox_rec import cox_rec
from ...aft.mle import mle
from ...aft.inference import inference as _aft_inference
...
def main(N, seed, data_setting, knots_setting='quantile', ci=False, root=None):
    ...
    if ci:
        se_all = _aft_inference(N, seed, data_setting, knots_setting, root=root)
