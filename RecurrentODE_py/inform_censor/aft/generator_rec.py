"""Generator for the ``inform_censor`` AFT recurrent-event setting.

* Recurrent event intensity (AFT, setting 2):
      lambda(t|x) = 2 * exp(x'beta) / sqrt(4 * exp(x'beta) * t + 1),
  beta = (1, 1, 1).
* Censoring time comes from a Cox survival model conditional on the SAME
  covariates as the event process:
      C | x ~ Exp( lambda_0_C * exp(x' gamma) ),
  so (C, N(.)) are conditionally independent given x but marginally
  correlated via x.
* Fixed upper bound TAU caps follow-up at ``min(C, TAU)``. Imperceptible
  per-subject jitter (<= 1e-6, far below any real time scale) is added
  only to subjects at the cap / below the numerical floor, so that the
  solve_ivp in ``aft.cox_rec`` sees a strictly-increasing t_eval.

File format matches ``aft/generator_rec.py`` exactly, so ``aft.cox_rec``,
``aft.mle`` and ``aft.inference`` work unchanged.
"""
...
def generator_rec(N, seed, data_setting, data_dir=None,
    ...
    # Subject i's censoring time uses the SAME x[i] as its event intensity.
