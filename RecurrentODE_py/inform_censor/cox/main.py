"""Driver for the informative-censoring Cox recurrent-event simulation.

Identical fit/inference code as ``RecurrentODE_py.cox.main`` -- we only
swap the data generator for ``inform_censor.cox.generator_rec`` so that
censoring is drawn from a Cox survival model C|x ~ Exp(lambda_0 * exp(x'gamma)).

On-disk layout:
    inform_censor/cox/data/simudata_N*_seed*_setting*.npz
    inform_censor/cox/res/res_cox_N*_seed*_setting*[_se].npz
so that the standard ``test_coverage`` aggregation plugs in unchanged.
"""
...
def main(N, seed, data_setting, calculate_ci=False, root=None):
    ...
    if calculate_ci:
        # cox.inference reads <root>/data/simudata_... and
        # <root>/res/res_cox_... -- both already point at inform_censor/cox/*.
