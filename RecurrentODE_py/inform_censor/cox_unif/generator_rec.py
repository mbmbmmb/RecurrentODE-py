...
* Recurrent event intensity (Cox, setting 1):
      lambda(t|x) = (t^2 + 1) * exp(x'beta),   beta = (1, 1, 1).
* Censoring time C|x ~ Uniform(a(x), b(x)) with simple linear bounds:
  ...
  so this preserves the conditional non-informative censoring assumption.
  Marginal correlation between C and N(.) arises through x.
* With x1, x2 truncated to [-1, 1] and x3 in {0, 1}, x' gamma in [-2, 3]
  so b(x) in [2.5, 5] (always > a(x)). Higher x' gamma -> smaller upper
  bound -> earlier censoring on average -- the same direction of
  dependence as the Cox- and AFT-survival censoring variants.
* No admin-cap TAU here: C is already bounded by max b(x) = 5. A
  numerical floor U_MIN protects downstream solvers; ties at the floor
  get imperceptible (<= 1e-6) jitter to keep solve_ivp's t_eval strictly
  increasing.
"""
...
def generator_rec(N, seed, data_setting, data_dir=None,
    ...
    # safety margin: x is clipped to a known support so this never fires
    # for truncN(0,1)*[-1,1] x covariates, but guard anyway.
