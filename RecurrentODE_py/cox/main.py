...
Runs a single replication of the Cox-type recurrent-event simulation: load or
generate data, fit the model by ``scipy.optimize.minimize`` with exact
gradient, and save the estimates (plus SEs if requested) to ``res/``.
"""
...
def main(N, seed, data_setting, calculate_ci=False, root=None):
    ...
    k = 3
    l = x.shape[0] ** (1.0 / 7.0)
    # MATLAB: augknt(linspace(0, max(time), l+1), k). l is non-integer in MATLAB
    # too; linspace uses ceil-ish behaviour via floor+1. Use round as MATLAB does.
    ...
    if calculate_ci:
        se_all = inference(N, seed, data_setting, root=root)
