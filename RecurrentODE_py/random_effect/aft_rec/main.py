"""Port of random effect/aft_rec/main.m."""
...
def main(N, seed, data_setting, knots_setting='quantile', ci=False, root=None):
    ...
    temp, beta_init = cox_rec(x, time, delta)
    ...
    if knots_setting == 'quantile':
        ...
        knots = augknt(
            np.concatenate([[0.0], interior, [float(np.max(temp))]]), k,
        )
    ...
    est, runtime = mle(x, time, delta, id_vec, knots, k, r0, forward)
    print(f'Simulation:{seed} finished with time:{runtime:.3f}')
    ...
    np.savez_compressed(
        out_path,
        est_r=est.reshape(-1, 1), runtime=runtime,
